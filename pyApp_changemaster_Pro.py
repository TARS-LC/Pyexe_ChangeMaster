...
class PyAppChanger(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        # --- ファイル設定エリア ---
        frame_file = ttk.LabelFrame(self, text="ファイル・環境設定")
        ...
        # --- オプションエリア ---
        frame_opt = ttk.LabelFrame(self, text="オプション")
        ...
        ttk.Checkbutton(frame_opt, text="spyder-runtimeを同梱する (exe本体の横にポータブルPythonをコピー)", variable=self.bundle_runtime_var).grid(row=2, column=0, columnspan=2, sticky=tk.W, **padding_opts)
        ...
        # --- 進行状態エリア ---
        frame_prog = ttk.LabelFrame(self, text="進行状態")
...
"""
    ...
    def auto_detect_data(self, silent=True):
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Constant) and isinstance(node.value, str):
                    ...
                    if 2 < len(val) < 200 and '\n' not in val:
                        if 'spyder' in val.lower(): continue
                        if '.' in val or '/' in val:
                            literals.add(val)
        except Exception as e:
        ...
        file_map = {}
        exclude_dirs = {'__pycache__', '.git', '.vscode', '.idea', 'venv', 'env', '.venv', 'build', 'dist', 'node_modules', 'spyder_runtime'}
        
        for root, dirs, files in os.walk(base_dir):
            dirs[:] = [d for d in dirs if d.lower() not in exclude_dirs and not d.startswith('.') and 'spyder' not in d.lower()]
            for file in files:
                if file.lower() == target_py_name or file.endswith(('.py', '.pyc', '.spec')) or 'spyder' in file.lower():
                    continue
    ...
    def _run_conversion(self, py_file, out_dir):
        try:
            ...
            portable_python_dir = os.path.join(app_dir, "spyder-runtime")
            ...
            # Conda系(spyder-runtime)環境特有のDLLパス自動追加
            ...
            # --- 7. spyder-runtime の同梱処理 ---
            if self.bundle_runtime_var.get():
                self.log(">>> spyder-runtime を同梱中 (コピーしています...)")
                dest_runtime = os.path.join(dist_path, "spyder-runtime")

                if os.path.exists(portable_python_dir):
                    try:
                        shutil.copytree(portable_python_dir, dest_runtime)
                        self.log(">>> spyder-runtime の同梱が完了しました。")
                    except Exception as e:
                        self.log(f"[警告] spyder-runtime のコピーに失敗しました: {e}")
                else:
                    self.log("[警告] ツール側に spyder-runtime フォルダが見つからないため、同梱をスキップします。")
